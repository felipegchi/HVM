...
pub fn new_rulebook(extra_funs: &[Precomp]) -> RuleBook {
...
pub fn gen_rulebook(file: &lang::File, extra_funs: &[Precomp]) -> RuleBook {
  // Creates an empty rulebook
  let mut book = new_rulebook(extra_funs);
  ...
  fn sanitize_term(
    ...
    let term = match term {
      ...
      lang::Term::Sup { val0, val1 } => {
        ...
        let term = lang::Term::Sup { val0, val1 };
        Box::new(term)
      }
      ...
      lang::Term::U6O { numb } => {
        let term = lang::Term::U6O { numb: *numb };
        Box::new(term)
      }
      lang::Term::F6O { numb } => {
        let term = lang::Term::F6O { numb: *numb };
        Box::new(term)
      }
    };
...
mod tests {
  ...
  use hvm_parser::{read_file, read_rule};
    ...
    if let (
      ...
      for (a_arg, b_arg) in a_args.iter().zip(b_args) {
        match **a_arg {
          ...
          lang::Term::U6O { numb: a_arg_numb } => match **b_arg {
            lang::Term::U6O { numb: b_arg_numb } => {
          ...
          lang::Term::F6O { numb: a_arg_numb } => match **b_arg {
            lang::Term::F6O { numb: b_arg_numb } => {
              if a_arg_numb != b_arg_numb {
                return (false, false);
              }
            }
                ...
                if let (
                  ...
                  for (rule_arg, other_arg) in rule_args.iter().zip(other_args) {
                    match &**rule_arg {
                      ...
                      lang::Term::U6O { numb: rule_arg_numb } => {
                        match &**other_arg {
                          lang::Term::U6O { numb: other_arg_numb } => {
                      ...
                      lang::Term::F6O { numb: rule_arg_numb } => {
                        match &**other_arg {
                          lang::Term::F6O { numb: other_arg_numb } => {
                            if rule_arg_numb == other_arg_numb {
                              other_new_lhs_args.push(Box::new(*other_arg.clone()));
                            } else {
                              panic!("Internal error. Please report."); // not possible since it matches
